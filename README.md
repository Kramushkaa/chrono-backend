...
- **TypeScript 5.7** - типизированный JavaScript (100% type coverage), target ES2024
...
```bash
npm install
# shared-dto собирается как npm workspace и доступен как @chrononinja/dto
npm run --workspace shared-dto build
```
...
### Диагностика кэша метаданных

Для наблюдения за состоянием in-memory кэша метаданных используйте:

```bash
# Параметр CACHE_INSPECT_URL позволяет указать иной URL (по умолчанию http://localhost:3001/api/cache/stats)
CACHE_INSPECT_URL=http://localhost:3001/api/cache/stats npm run cache:inspect
```

Скрипт обращается к административному эндпоинту `/api/cache/stats` и выводит текущий список ключей, TTL и актуальные ETag значения.
