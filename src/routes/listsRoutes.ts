...
export function createListsRoutes(pool: Pool): Router {
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      const own = await pool.query('SELECT owner_user_id, title FROM lists WHERE id = $1', [
        listId,
      ]);
      ...
      const code = jwt.sign({ listId: Number(listId), owner: String(userId) }, secret, {
        expiresIn: '365d',
      });
  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      const { listId } = req.params;
      const own = await pool.query('SELECT 1 FROM lists WHERE id = $1 AND owner_user_id = $2', [
        listId,
        req.user?.sub,
      ]);
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      if (!['person', 'achievement', 'period'].includes(item_type))
      ...
      if (item_type === 'person') {
        ...
        const exists = await pool.query(
          ...
          [listId, 'person', person_id]
        );
        if (((exists && (exists as any).rowCount) || 0) > 0) {
          res.status(200).json({ success: true, data: exists.rows[0], message: 'already_exists' });
          return;
        }
        ...
        const exists = await pool.query(
          ...
          [listId, 'achievement', achievement_id]
        );
        if (((exists && (exists as any).rowCount) || 0) > 0) {
          res.status(200).json({ success: true, data: exists.rows[0], message: 'already_exists' });
          return;
