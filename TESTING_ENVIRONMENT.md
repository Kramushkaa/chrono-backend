...
Если `DB_SCHEMA` не указана, сервер автоматически использует `public`. Для тестовой среды **обязательно** задайте `DB_SCHEMA=test`, чтобы данные не попадали в продакшн.
