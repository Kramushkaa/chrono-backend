...
- ✅ `dotenv` 16.6.1 → 17.2.3 **(low risk major update)**
...
2. ✅ **Выполнено:** `dotenv` 16 → 17
3. ❌ **Отложено:** `react-helmet-async` 1 → 2 (оказалось высокий риск)
...
### Этап 2 (текущая сессия):
- ✅ Обновлен `dotenv` до v17.2.3
- ✅ Протестировано (624/624 тестов)
- ✅ Закоммичено и запушено
- ❌ `react-helmet-async` отложено (379 ошибок TypeScript)

### Этап 3 (следующие шаги):
1. Исследовать проблему с `react-helmet-async` v2
2. Обновить `helmet` до v8 (backend)
3. Обновить `react-router-dom` до v7 (frontend)

### Этап 4 (планирование - приоритет security):
1. Миграция с `node-telegram-bot-api` на `telegraf` (критично)
2. Планирование обновления Express и React при стабилизации
